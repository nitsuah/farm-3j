...
export const EnemySiegeCastersLayer: React.FC<EnemySiegeCastersLayerProps> = ({
  ...
  return (
    <>
      ...
      {enemySiege
        .filter(r => r.hp > 0 && fogVisible[Math.round(r.x)]?.[Math.round(r.y)])
        .map(r => {
      ...
      {/* Enemy Shamans */}
      {enemyShamans
        .filter(s => s.hp > 0 && fogVisible[Math.round(s.x)]?.[Math.round(s.y)])
        .map(s => {
      ...
      {/* Necromancers */}
      {enemyNecromancers
        .filter(n => n.hp > 0 && fogVisible[Math.round(n.x)]?.[Math.round(n.y)])
        .map(n => {
      ...
      {/* Witch Doctors */}
      {enemyWitchDoctors
        .filter(d => d.hp > 0 && fogVisible[Math.round(d.x)]?.[Math.round(d.y)])
        .map(wd => {
        ...
        .map(s => {
          ...
          return (
            ...
            >
              ...
              />
              {/* Fuse spark */}
              <text x={isoX + TILE_SIZE / 2 + 6} y={isoY + 28} fontSize="10">
                ✨
              </text>
              {/* Urgency pulse */}
              {urgency && (
