        ...
        >
          ...
          {(() => {
            const TOWER_RANGES: Partial<Record<BuildingType, number>> = {
              watchtower: guardTowerResearched ? WATCHTOWER_ATTACK_RANGE + 1 : WATCHTOWER_ATTACK_RANGE,
            ...
            const TOWER_COLORS: Partial<Record<BuildingType, string>> = {
              watchtower: '#94a3b8', frostTower: '#93c5fd', ballista: '#fbbf24', poisonTower: '#86efac',
            };
            return placedBuildings.filter(b => b.hp > 0 && !b.constructing && TOWER_RANGES[b.type] !== undefined).map(b => {
              const r = TOWER_RANGES[b.type]!;
              const { isoX, isoY } = tileToSvg(b.x, b.y);
              const cx = isoX + TILE_SIZE, cy = isoY + TILE_SIZE / 2;
              return <ellipse key={`range-${b.id}`} cx={cx} cy={cy} rx={r * TILE_SIZE} ry={r * TILE_SIZE / 2}
                fill="none" stroke={TOWER_COLORS[b.type] ?? '#94a3b8'} strokeWidth={1.5} strokeDasharray="6 4" opacity={0.25} pointerEvents="none" />;
            });
          })()}
          ...
          {buildMode && ghostTile && ghostTile.x >= 0 && ghostTile.y >= 0 && ghostTile.x < GRID_SIZE && ghostTile.y < GRID_SIZE && (() => {
            ...
            const GHOST_RANGES: Partial<Record<BuildingType, { r: number; color: string }>> = {
            ...
            const gcx = isoX + TILE_SIZE, gcy = isoY + TILE_SIZE / 2;
            return (
              <g pointerEvents="none">
                {ghostRange && <ellipse cx={gcx} cy={gcy} rx={ghostRange.r * TILE_SIZE} ry={ghostRange.r * TILE_SIZE / 2}
                  fill={`${ghostRange.color}15`} stroke={ghostRange.color} strokeWidth={2} strokeDasharray="8 4" opacity={0.7} />}
