// Per-frame worker state-machine tick � extracted from useGameLoop.tsx
// Do not change game logic here; this is a structural extraction only.
...
export function tickWorkers(
  ...
  const getMoraleMs = (wx: number, wy: number) => {
    ...
    const shoutMult =
      shouting && tileDist(wx, wy, heroUnit.x, heroUnit.y) <= HERO_SHOUT_RADIUS
        ? HERO_SHOUT_ATK_MULT
        : 1;
  ...
  return alive.map(w => {
    ...
    if (w.attackMove && w.state === 'moving' && !w.attacking) {
      ...
      const nearSapper = enemySappersRef.current.find(
        s => s.hp > 0 && !s.exploded && tileDist(w.x, w.y, s.x, s.y) <= AM_SCAN
      );
      if (nearSapper)
        return {
          ...w,
    ...
    if (w.holdPosition) {
      if (
        ...
        const nearSapperH = enemySappersRef.current.find(
          s =>
            s.hp > 0 && !s.exploded && tileDist(w.x, w.y, s.x, s.y) <= HP_RANGE
        );
        if (nearSapperH)
          return {
            ...w,
        ...
        const nearWCH = enemyWarchiefsRef.current.find(
          wc2 => wc2.hp > 0 && tileDist(w.x, w.y, wc2.x, wc2.y) <= HP_RANGE
        );
        if (nearWCH)
          return {
            ...w,
    ...
    if (
      ...
      const nearSapperA = enemySappersRef.current.find(
        s =>
          s.hp > 0 && !s.exploded && tileDist(w.x, w.y, s.x, s.y) <= AGG_RANGE
      );
      if (nearSapperA)
        return {
          ...w,
    ...
    if (w.movingTo) {
      ...
      if (d < epsilon) {
        ...
        if (w.state === 'returning') {
          ...
          const atBarn =
            tileDist(movDest.x, movDest.y, BARN_POS.x, BARN_POS.y) < epsilon;
          const atLumberShed =
            !atBarn &&
            w.carrying.lumber > 0 &&
            ...
            setEnemyGrunts(gs =>
              gs.map(eg =>
                eg.id === g.id
                  ? {
                      ...eg,
                      hp: Math.max(0, eg.hp - CAVALRY_TRAMPLE_DAMAGE * dt),
                    }
                  : eg
              )
            );
      ...
      if (gType === 'tree') {
        ...
        if (!gatherT[w.id]) {
          ...
          gatherT[w.id] = window.setTimeout(() => {
            ...
            setWorkers(ws2 =>
              ws2.map(w2 => {
                if (w2.id !== w.id || w2.state !== 'gathering' || !w2.gathering)
                  return w2;
                if (
                  ...
                  setTrees(ts =>
                    ts.map((t, i) => {
                      ...
                      if (next === 0)
                        addFloatingText(t.x, t.y, '🌲 Depleted!', '#92400e');
                  ...
                  const dropSite2 =
                    sheds2.reduce<{ x: number; y: number } | null>(
                      (best, s) => {
                        ...
                        return !best || d < tileDist(w2.x, w2.y, best.x, best.y)
                          ? { x: s.x, y: s.y }
                          : best;
                      },
                      null
        ...
        if (!gatherT[w.id]) {
          const gatherMs = Math.max(
            400,
            (GATHER_INTERVAL_MS - upgradesRef.current.swiftHarvest * 200) /
            ...
            setWorkers(ws2 =>
              ws2.map(w2 => {
                ...
                if (mine && mine.amount > 0 && w2.carrying.gold < CARRY_CAP) {
                  ...
                  const goldDrop =
                    camps.reduce<{ x: number; y: number } | null>((best, c) => {
        ...
        if (w.carrying.stone >= CARRY_CAP) {
          ...
          const stoneDrop =
            stoneCamps.reduce<{ x: number; y: number } | null>((best, c) => {
    ...
    if (w.state === 'attacking' && w.attacking) {
      if (w.attacking.targetType === 'creep') {
        ...
        if (!attackT[w.id]) {
          ...
          const unitBonusC =
            w.unitType === 'hero'
              ? HERO_DAMAGE_BONUS +
                heroItemsRef.current.reduce(
                  (s, it) => s + (HERO_ITEM_DATA[it.itemId].dmgBonus ?? 0),
                  0
                )
        ...
        const target = enemyGruntsRef.current.find(
          g =>
            g.id ===
            (w.attacking as { targetType: 'grunt'; gruntId: number }).gruntId
        );
        ...
        if (!attackT[w.id]) {
          ...
          attackT[w.id] = window.setTimeout(() => {
            ...
            setEnemyTowers(ts =>
              ts.map(t =>
                t.id === capturedTId ? { ...t, hp: Math.max(0, t.hp - dmg) } : t
              )
            );
        ...
        const lkId = (w.attacking as { targetType: 'lurker'; lurkerId: number })
          .lurkerId;
        ...
        if (!attackT[w.id]) {
          ...
          attackT[w.id] = window.setTimeout(() => {
            ...
            setWorkers(ws2 =>
              ws2.map(w2 => {
                if (w2.id !== w.id || w2.state !== 'attacking' || !w2.attacking)
                  return w2;
                setEnemyBarnHp(hp => {
    ...
    if (w.unitType === 'catapult' && w.state === 'idle' && !attackT[w.id]) {
      const nearestGrunt = enemyGruntsRef.current.reduce<EnemyGrunt | null>(
        (best, g) => {
    ...
    if (
      ...
      const burningBuilding = placedBuildingsRef.current.find(
        b =>
          b.hp > 0 && b.hp / b.maxHp < 0.25 && tileDist(w.x, w.y, b.x, b.y) <= 3
      );
      if (burningBuilding) {
